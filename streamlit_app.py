...
def prever_mercado_gols( minuto, placar_a, placar_b, finalizacoes_a, finalizacoes_b, chutes_alvo_a, chutes_alvo_b, xg_a, xg_b, ataques_perigosos_a, ataques_perigosos_b, posse_a, posse_b ): gols_total = placar_a + placar_b
...
st.set_page_config(page_title="Bot de Gols ao Vivo", layout="centered") st.title("⚽ Bot de Gols ao Vivo")

st.header("📋 Informações da Partida") minuto = st.slider("Minuto de jogo", 1, 90, 30) placar_a = st.number_input("Gols Time A", min_value=0) placar_b = st.number_input("Gols Time B", min_value=0)

st.header("📊 Estatísticas do Time A") finalizacoes_a = st.number_input("Finalizações Time A", min_value=0) chutes_alvo_a = st.number_input("Chutes no Alvo Time A", min_value=0) xg_a = st.number_input("xG Time A", min_value=0.0) ataques_perigosos_a = st.number_input("Ataques Perigosos Time A", min_value=0) posse_a = st.slider("Posse de Bola Time A (%)", 0, 100, 50)

st.header("📊 Estatísticas do Time B") finalizacoes_b = st.number_input("Finalizações Time B", min_value=0) chutes_alvo_b = st.number_input("Chutes no Alvo Time B", min_value=0) xg_b = st.number_input("xG Time B", min_value=0.0) ataques_perigosos_b = st.number_input("Ataques Perigosos Time B", min_value=0) posse_b = 100 - posse_a  # automático

if st.button("🔍 Analisar Jogo"): resultado = prever_mercado_gols( minuto, placar_a, placar_b, finalizacoes_a, finalizacoes_b, chutes_alvo_a, chutes_alvo_b, xg_a, xg_b, ataques_perigosos_a, ataques_perigosos_b, posse_a, posse_b )
