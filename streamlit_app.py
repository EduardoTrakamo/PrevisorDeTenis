...
st.header("💸 Odds do Mercado")
odd_over15 = st.number_input("Odd Over 1.5", min_value=1.01)
odd_over25 = st.number_input("Odd Over 2.5", min_value=1.01)
...
if st.button("🔍 Analisar Jogo"):
    ...
    st.markdown(f"**🎯 Over 1.5 → Prob:** `{resultado['prob_over15']*100:.1f}%` | **EV:** `{resultado['ev_over15']:.2f}`")
    st.markdown(f"**🎯 Over 2.5 → Prob:** `{resultado['prob_over25']*100:.1f}%` | **EV:** `{resultado['ev_over25']:.2f}`")
    ...
    if resultado['ev_over25'] > 0:
        st.success("✅ Over 2.5 tem valor! Possível entrada.")
    else:
        st.warning("❌ Over 2.5 sem valor no momento.")
